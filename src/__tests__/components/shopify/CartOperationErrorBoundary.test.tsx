...
describe("CartOperationErrorBoundary", () => {
  ...
  describe("Component Rendering", () => {
    ...
    it("renders error fallback when error occurs", () => {
      const { getByText } = render(
        <CartOperationErrorBoundary>
          <ThrowError shouldThrow={true} error={new Error("Cart operation failed")} />
        </CartOperationErrorBoundary>
      );

      expect(getByText("Cart Operation Failed")).toBeTruthy();
      expect(getByText(/We encountered an issue updating your cart/)).toBeTruthy();
    });
  });
  ...
  describe("Error Classification", () => {
    it("identifies network errors correctly", () => {
      const { getByText } = render(
        <CartOperationErrorBoundary>
          <ThrowError shouldThrow={true} error={new Error("Network timeout occurred")} />
        </CartOperationErrorBoundary>
      );

      expect(getByText("Connection Issue")).toBeTruthy();
      expect(getByText(/Unable to update your cart due to connection issues/)).toBeTruthy();
    ...
    it("identifies inventory errors correctly", () => {
      const { getByText } = render(
        <CartOperationErrorBoundary>
          <ThrowError shouldThrow={true} error={new Error("Item out of stock")} />
        </CartOperationErrorBoundary>
      );

      expect(getByText("Item Unavailable")).toBeTruthy();
      expect(getByText(/The item you're trying to add is no longer available/)).toBeTruthy();
      expect(getByText(/Check product availability and try alternative options/)).toBeTruthy();
    });

    it("identifies validation errors correctly", () => {
      const { getByText } = render(
        <CartOperationErrorBoundary>
          <ThrowError shouldThrow={true} error={new Error("Validation failed: invalid size")} />
        </CartOperationErrorBoundary>
      );

      expect(getByText("Cart Validation Error")).toBeTruthy();
      expect(getByText(/There was an issue with your cart selection/)).toBeTruthy();
    });

    it("identifies conflict errors correctly", () => {
      const { getByText } = render(
        <CartOperationErrorBoundary>
          <ThrowError shouldThrow={true} error={new Error("Concurrent modification conflict")} />
        </CartOperationErrorBoundary>
      );

      expect(getByText("Cart Sync Issue")).toBeTruthy();
      expect(getByText(/Your cart was updated from another device/)).toBeTruthy();
      expect(getByText(/Cart was updated from another device - syncing latest version/)).toBeTruthy();
    });
  ...
  describe("Dynamic Button Rendering", () => {
    it("shows 'Sync Cart' button for conflict errors", () => {
      const { getByText } = render(
        <CartOperationErrorBoundary>
          <ThrowError shouldThrow={true} error={new Error("Version conflict detected")} />
        </CartOperationErrorBoundary>
      );
  ...
  describe("Callback Handling", () => {
    ...
    it("calls onInventoryError callback for inventory errors", () => {
      ...
      render(
        <CartOperationErrorBoundary onInventoryError={mockOnInventoryError}>
          <ThrowError shouldThrow={true} error={new Error("Item is out of stock")} />
        </CartOperationErrorBoundary>
      );
    ...
    it("calls onSyncConflict callback for conflict errors", () => {
      ...
      render(
        <CartOperationErrorBoundary onSyncConflict={mockOnSyncConflict}>
          <ThrowError shouldThrow={true} error={new Error("Concurrent update conflict")} />
        </CartOperationErrorBoundary>
      );
    ...
    it("calls multiple callbacks when applicable", () => {
      ...
      render(
        ...
        >
          <ThrowError shouldThrow={true} error={new Error("Product unavailable")} />
        </CartOperationErrorBoundary>
      );
  ...
  describe("Error Type Variants", () => {
    ...
    testCases.forEach(({ error, expectedTitle, expectedHint }) => {
      it(`correctly handles ${error.message.includes('network') ? 'network' : 
        error.message.includes('inventory') || error.message.includes('stock') ? 'inventory' :
        error.message.includes('validation') || error.message.includes('invalid') ? 'validation' :
  ...
  describe("Accessibility and Layout", () => {
    ...
    it("handles very long error messages", () => {
      const longError = new Error("This is a very long error message that might cause layout issues if not handled properly in the UI component rendering system");
