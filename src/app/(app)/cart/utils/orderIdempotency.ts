...
export function generateOrderIdempotencyKey(
  ...
  const itemIds = cartItems.map(item => `${item.productId}:${item.selectedQuantity}`).sort().join('|');
 ...
 */
export async function saveOrderIdempotencyKey(idempotencyKey: string): Promise<void> {
  try {
...
export async function createOrderIdempotent(
  ...
  const orderKey = idempotencyKey || generateOrderIdempotencyKey(
    userId, 
    orderData.items, 
  ...
  if (existingOrderRef) {
    ...
    console.log('Found existing order with same idempotency key, preventing duplicate');
...
async function checkForExistingOrder(
  ...
  try {
    ...
    if (!querySnapshot.empty) {
      ...
      console.log(`Found existing order with idempotency key ${idempotencyKey}: ${docSnapshot.id}`);
...
export async function reconcileOrder(
  ...
  try {
    ...
    if (!querySnapshot.empty) {
      console.log(`Found ${querySnapshot.size} orders with payment intent ID ${paymentIntentId}`);
    ...
    if (idempotencyKey) {
      console.log('Checking for order with stored idempotency key:', idempotencyKey);
      const idempotencyQuery = query(ordersRef, where('idempotencyKey', '==', idempotencyKey));
      const idempotencySnapshot = await getDocs(idempotencyQuery);
      
      if (!idempotencySnapshot.empty) {
        console.log(`Found ${idempotencySnapshot.size} orders with idempotency key ${idempotencyKey}`);
...
export async function attemptOrderRecovery(
  ...
  try {
    ...
    if (paymentIntentId) {
      const paymentIntentOrders = await reconcileOrder(db, userId, paymentIntentId);
    ...
    const idempotencyKey = await getOrderIdempotencyKey();
    if (idempotencyKey) {
      console.log('Attempting recovery with stored idempotency key:', idempotencyKey);
      const ordersRef = collection(db, 'users', userId, 'orders');
      const idempotencyQuery = query(ordersRef, where('idempotencyKey', '==', idempotencyKey));
      const idempotencySnapshot = await getDocs(idempotencyQuery);
