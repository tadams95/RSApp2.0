...
describe("Realtime Database Sync Utilities", () => {
  ...
  describe("calculateBackoffDelay", () => {
    ...
    it("should respect maxBackoffDelay", () => {
      ...
      expect(attempt10).toBeGreaterThanOrEqual(
        DEFAULT_SYNC_OPTIONS.maxBackoffDelay!
      );
      expect(attempt10).toBeLessThan(
        DEFAULT_SYNC_OPTIONS.maxBackoffDelay! * 1.1
      ); // With jitter
    });
  ...
  describe("handleSyncError", () => {
    ...
    it("should call onError callback if provided", () => {
      ...
      handleSyncError(mockError, "users/123/profile", 1, {
        onError: onErrorMock,
      });
  ...
  describe("executeWithRetry", () => {
    ...
    it("should throw after maximum retries", async () => {
      ...
      await expect(
        executeWithRetry(mockOperation, "test/path", {
          maxRetries: 2,
