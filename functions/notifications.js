...
exports.onPostLikeCreateNotify = onDocumentCreated(
  "postLikes/{likeId}",
  async (event) => {
    try {
...
exports.onPostCommentCreateNotify = onDocumentCreated(
...
exports.onPostCreateNotifyMentions = onDocumentCreated(
  "posts/{postId}",
  async (event) => {
    try {
      ...
      if (!authorId || !postId || !content || typeof content !== "string")
        return null;
...
exports.onFollowCreateNotify = onDocumentCreated(
  "follows/{followId}",
  async (event) => {
    try {
...
exports.onRepostCreateNotify = onDocumentCreated(
...
exports.batchMarkNotificationsRead = onCall(async (request) => {
  ...
  if (markAll) {
    ...
    const unique = [
      ...new Set(notificationIds.filter((x) => typeof x === "string")),
    ].slice(0, HARD_CAP);
    if (unique.length) {
  ...
  await db.runTransaction(async (tx) => {
    ...
    if (decrement > 0) {
      tx.update(userRef, {
        unreadNotifications: Math.max(0, currentUnread - decrement),
      });
    }
  });
...
exports.onUserNotificationCreatedSendPush = onDocumentCreated(
  ...
  async (event) => {
    ...
    try {
      if (!notif.sendPush) {
        logger.debug("sendPush disabled on notification", {
          uid,
      ...
      if (prefs && prefs[notif.type] === false) {
        logger.debug("Notification type disabled in prefs", {
          uid,
          type: notif.type,
        });
          ...
          try {
            ...
            if (inQuiet) {
              await notifSnap.ref.update({
                pushStatus: "suppressed_quiet_hours",
              });
              return null;
            }
          } catch (qhErr) {
      ...
      if (
        ["post_liked", "comment_added"].includes(notif.type) &&
        notif.data?.postId
      ) {
        try {
          ...
          if (relevant.length > 1) {
            ...
            const actorsPart =
              actorIds.size > 1 ? `${actorIds.size} people` : "Someone";
            if (likeCount && commentCount) {
              aggregatedTitle = "New activity on your post";
  ...
  async () => {
    ...
    try {
      ...
      snap.docs.forEach((doc) => {
        ...
        const d = doc.data() || {};
        const lastSeen =
          d.lastSeenAt && d.lastSeenAt.toDate ? d.lastSeenAt.toDate() : null;
        const created =
...
exports.onNotificationPrefsWrittenSanitize = onDocumentWritten(
  ...
  async (event) => {
    try {
      ...
      logger.error("onNotificationPrefsWrittenSanitize failed", {
        err,
